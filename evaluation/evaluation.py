...
def _build_camera_eval_command(
    ...
    if args.disable_global_mapper:
        cmd.append("--disable_global_mapper")
    if args.verbose:
        cmd.append("--verbose")
...
def main() -> None:
    ...
    if pair_metric_names:
        for video_dir in video_dirs:
            ...
            for cam_video in cam_videos:
                ...
                for metric_name in pair_metric_names:
    ...
    print(f"[pipeline] Wrote report: {output_path}")
    print(f"[pipeline] num_video_dirs: {summary['num_video_dirs']}")
    print(f"[pipeline] selected_metrics: {selected_metrics}")
